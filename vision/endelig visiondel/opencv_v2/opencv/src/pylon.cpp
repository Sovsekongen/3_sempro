...
void pubCallback(const std_msgs::Empty::ConstPtr& robotState){
   ...
   // camera_info_manager::CameraInfoManager a(nh, "camera", "/home/mads/catkinws/src/pylon_camera/config/default.yaml");
    ...
    //sørger for at bolden bliver fundet på det undistortet billede
    if(trackFilteredObject2(x, y, radius, threshold2, result)){
        ...
        if(ros::ok){
          ...
          //gemmer billedet på computeren
          ifstream in;
          ofstream out;
          in.open("/home/mads/Documents/Semesterprojekt/imageUpdate");
          if(!in.is_open()){
          ...
          out.open("/home/mads/Documents/Semesterprojekt/imageUpdate");
          if(!out.is_open()){
              std::cout << "fucking fail" << std::endl;
              return -1;
          }
          imwrite("/home/mads/Documents/billeder/semesterprojekt/result.jpg", result);
